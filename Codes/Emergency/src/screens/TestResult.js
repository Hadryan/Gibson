...
import { Container, Row, Col,Text, Content, Item, } from 'native-base';
...
export default class TestResult extends React.Component {
    ...
    getData=()=>{
        ...
        firebase.firestore().collection('TestResult').where('username','==',user).get().then(snap=>{
            let data=[]
            snap.forEach(doc=>{
                data.push(doc.data())
            })
    ...
    renderItem=(item,i)=>{
        ...
        <Text style={{fontWeight:'bold',fontSize:18,color:'#352641',marginBottom:10}}>{item.date}</Text>
...
render() {
    return (
        ...
        <Content style={{marginTop:25}} refreshControl={<RefreshControl refreshing={this.state.refreshing} onRefresh={()=>this.getData()}/>}> 
